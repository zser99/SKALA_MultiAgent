...
| 담당자 | 담당 에이전트 | 작업 사항 |
| ------ | --------------------- | ------------------------------------------------ |
| 오상현 | 기술 조사 Agent | 원문 확보 및 기술 원리·범위·한계 추출 |
| 목진훈 | 시장 평가 Agent | 시장성·상용화·채택 현황 조사 |
| 오지연 | 이해관계자 평가 Agent | 개발자·경쟁사·산업계 등의 반응 조사 |
| 이은서 | 도메인 평가 Agent | 특정 Domain에서 기술 적합성 평가 |
| 이휘호 | 평가 종합 Agent | 관점별 결과의 일치/불일치 종합 |
| 이민서 | 보고서 생성 Agent | State의 결과를 최종 보고서로 구성 |
