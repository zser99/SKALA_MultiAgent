...
def _market_one(candidate: dict) -> tuple[str, str]:
    vs, _ = build_vectorstore(candidate["id"], candidate["file"])
    context = retrieve_context(vs, QUERY, k=4) if vs else ""
...
def market_node(state: dict) -> dict:
    sw_summary, sw_basis = _market_one(state["selected_sw"])
    hw_summary, hw_basis = _market_one(state["selected_hw"])
    return {
        "market_result": {
            "sw": sw_summary,
            "hw": hw_summary,
